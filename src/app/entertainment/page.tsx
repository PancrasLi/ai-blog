    ...
    <main className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-16 max-w-6xl">
        ...
        <div className="mb-16 space-y-4">
          <div>
            <h1 className="text-5xl font-bold text-foreground mb-3">H5 游戏</h1>
            <p className="text-sm text-muted-foreground font-medium">创意来自 TAL/好未来 龙虾大赛</p>
          </div>
          <p className="text-lg text-muted-foreground leading-relaxed">
            优秀的网页游戏收集和创意游戏展示
          </p>
          <div className="h-1 w-16 bg-gradient-to-r from-purple-500 to-transparent rounded-full mt-4" />
        </div>
            ...
            {games.map((game) => (
              <Link key={game.id} href={`/entertainment/${game.id}`}>
                <div className="group relative h-full">
                  ...
                  {/* 内容容器 */}
                  <div className="relative border border-border rounded-xl overflow-hidden hover:border-purple-500/50 hover:shadow-lg transition-all duration-300 cursor-pointer bg-background/50 backdrop-blur-sm flex flex-col h-full">
                    {/* 游戏缩略图 */}
                    {game.thumbnail && (
                      <div className="relative w-full h-48 bg-muted flex items-center justify-center overflow-hidden group-hover:scale-105 transition-transform duration-300">
                        <img
                          src={game.thumbnail}
                          alt={game.title}
                    ...
                    <div className="p-6 flex-1 flex flex-col gap-4">
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <h3 className="text-xl font-bold text-foreground group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors duration-200 line-clamp-2 flex-1">
                          {game.title}
                        </h3>
                        {game.difficulty && (
                      ...
                      <div className="flex items-center gap-4 text-sm text-muted-foreground pt-2">
                        {game.plays !== undefined && (
                          <div className="flex items-center gap-1.5">
                            <TrendingUp className="h-4 w-4 text-purple-500/60" />
                            <span className="font-medium">{game.plays} 次</span>
                          </div>
                        )}
                        {game.rating !== undefined && (
                          <div className="flex items-center gap-1.5">
                            <Star className="h-4 w-4 text-purple-500/60 fill-purple-500/60" />
