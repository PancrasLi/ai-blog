    ...
    <main className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-16 max-w-6xl">
        ...
        <div className="mb-16 space-y-4">
          <div>
            <h1 className="text-5xl font-bold text-foreground mb-3">教育游戏</h1>
            <p className="text-sm text-muted-foreground font-medium">创意来自 TAL/好未来 龙虾大赛</p>
          </div>
          <p className="text-lg text-muted-foreground leading-relaxed">
            精选益智教育游戏，通过游戏方式提升脑力、记忆、计算、反应等多维度能力
          </p>
          <div className="h-1 w-16 bg-gradient-to-r from-green-500 to-transparent rounded-full mt-4" />
        </div>
            ...
            {games.map((game) => (
              <Link key={game.id} href={`/education/games/${game.id}`}>
                <div className="group relative h-full">
                  ...
                  {/* 内容容器 */}
                  <div className="relative border border-border rounded-xl px-6 py-8 hover:border-green-500/50 hover:shadow-lg transition-all duration-300 cursor-pointer bg-background/50 backdrop-blur-sm flex flex-col h-full">
                    {/* 标题和难度 */}
                    <div className="flex items-start justify-between gap-3 mb-4">
                      <h3 className="text-xl font-bold text-foreground group-hover:text-green-600 dark:group-hover:text-green-400 transition-colors duration-200 line-clamp-2 flex-1">
                        {game.title}
                      </h3>
                    ...
                    {/* 益处 */}
                    <div className="bg-green-500/10 border border-green-500/30 rounded-lg px-3 py-2 mb-4 flex items-start gap-2">
                    ...
                    <div className="flex items-center gap-4 text-sm text-muted-foreground pt-2">
                      ...
                      {game.rating !== undefined && (
                        <div className="flex items-center gap-1.5">
                          <Star className="h-4 w-4 text-green-500/60 fill-green-500/60" />
                    ...
                    {/* 标签 */}
                    {game.tags.length > 0 && (
                      <div className="flex gap-2 flex-wrap pt-4 mt-4 border-t border-border">
                        {game.tags.map((tag) => (
                          <Badge
                            key={tag}
                            variant="outline"
